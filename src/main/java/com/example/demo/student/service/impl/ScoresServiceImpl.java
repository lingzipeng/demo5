...
import com.example.demo.student.repository.ScoresRepository;
...
import com.example.demo.student.vo.BarEchartsSeriesModel;
...
public class ScoresServiceImpl implements IScoresService {
     ...
     */
    @Override
    public HashMap<String, Object> allScoresCensus() {
        HashMap<String, Object> resultMap = new HashMap<>();
        // 获取该学科下的所有成绩记录
        List<Scores> scoresList = scoresRepository.findAll();
            ...
            for (int i = 0; i < scoresList.size(); i++) {
                ...
                if (scoresList.get(i).getStudent().getId() == j) {
                    ...
                    if (!student.containsKey("stuno")) {
                        student.put("stuno", scoresList.get(i).getStudent().getStuno());
                    }
                    if (!student.containsKey("classname")) {
                        student.put("classname", scoresList.get(i).getGradeClass().getName());
                    }
                }
            }
            if (!student.isEmpty()) {
                resultMap.put(j + "", student);
            }

//            if (!student.isEmpty()) {
//                student.put("name", scoresList.get(j).getStudent().getName());
//                resultMap.put(j + "", student);
//            }
        }
